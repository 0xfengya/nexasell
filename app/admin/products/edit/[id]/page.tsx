...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/products/${id}`)
      ...
      .catch(() => setApiErr("Gagal memuat data produk."))
  ...
  return (
    <div className="p-4 sm:p-6 w-full max-w-full">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-400 ml-3.5">{form.name}</p>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-3 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-4 py-3 rounded-xl mb-5 border border-emerald-200 dark:border-emerald-500/20">
          <CheckCircle className="w-4 h-4 flex-shrink-0"/><span className="text-sm font-semibold">Perubahan berhasil disimpan!</span>
        </div>
      )}
      {apiErr && (
